...
export const DiagramsNetFrame = (props)=>{
    const {data, onSaveCallback, onStopEditing} = props;
    const frameRef = useRef(null);
    const url = useMemo(buildURL, [data]);
    const frameId = useMemo(()=>`id-${Math.random()}`, [data]);
    const messageEventHandler = useCallback((eventData)=>{
        ...
        switch (eventData.event){
            ...
            case "export": return onSaveCallback(eventData);
            case "exit": return  onStopEditing();
